...
def encontrar_pessoa(elemento):
    ...
    for i in range(len(lista)):  # procurar em todas as listas internas
        for j in range(i):  # procurar em todos os elementos nessa lista
            for k in range(j):
                ...
                if elemento in lista[i][j][k]:  # se encontrar elemento
                    pos_i = i  # guarda o índice i
    ...
    return (pos_i, pos_j, pos_k)  # e retornamos os índices
...
a_preferencia = (lista[lista_interna][1])
